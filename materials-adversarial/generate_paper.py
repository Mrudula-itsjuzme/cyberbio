with open("docs/PAPER_DRAFT.md", "w") as f:
...
1.  **Ordinary Transformer (~12M params):** A baseline sequence-to-sequence style encoder operating on SMILES tokens. It was evaluated to establish baseline vulnerability.
2.  **Architecture Control (~12M params):** A two-branch variant attempting to separate syntax from semantics.
3.  **Mixed-robust Transformer (~12M params):** A transformer trained with adversarial data augmentation.
4.  **Augmented Transformer (~12M params):** Extensive augmentation attempt.
5.  **GraphMPNN (~5M params):** The **Canonical Model**. A Message Passing Neural Network operating directly on molecular graphs. It was selected for achieving the highest clean validation performance and structural invariance to serialization.
...
Domain generalization requires shared attack/search mechanics combined with domain-specific components: representation adapter, validity checker, constraints, operators, and oracle. While chemistry uses SMILES/graphs and DFT oracles, this framework architecture seamlessly scales to DNA (nucleotides, biological constraints) and proteins (amino-acid sequences/structures, biophysical reference). 
